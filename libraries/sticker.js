...
/**
 * Image to Sticker using Sharp (Faster & Lower Memory)
 */
async function stickerSharp(img, url) {
  if (url) {
    const res = await fetch(url)
    if (res.status !== 200) throw await res.text()
    img = await res.buffer()
  }
  const type = await fileTypeFromBuffer(img) || { mime: 'application/octet-stream' }
  if (!/image/i.test(type.mime)) throw 'Not an image'
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const ff = spawn('ffmpeg', [
        '-y',
        '-i', inp,
        '-vf', 'scale=512:512:flags=bicubic:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000,setsar=1',
        '-f', 'png',
        '-'
      ])
      ff.on('error', reject)
  ...
  return new Promise(async (resolve, reject) => {
    ...
    Fffmpeg
      .on('error', function (err) {
        console.error(err)
        fs.promises.unlink(tmpFile).catch(console.error)
        reject(img)
      })
...
async function addExif(webpSticker, packname, author, categories = [''], metadata = {}) {
  ...
  webpSticker = null; // Memory Cleanup
  return result
}
...
async function sticker(img, url, ...args) {
  ...
  for (const func of [
    stickerSharp, sticker3, global.support.ffmpeg && sticker6, sticker5,
