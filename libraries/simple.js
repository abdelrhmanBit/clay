...
export function makeWASocket(connectionOptions, options = {}) {
	/**
	   * @type {
	   ...
	   | 
	   import("@whiskeysockets/baileys").WALegacySocket
	   }
	   */
	...
	const sock = Object.defineProperties(conn, {
		...
		getFile: {
			...
			async value(PATH, saveToFile = false) {
				let res;
				let filename;
				const data = Buffer.isBuffer(PATH) ? PATH : PATH instanceof ArrayBuffer ? PATH.toBuffer() : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await fetch(PATH)).buffer() : fs.existsSync(PATH) ? (filename = PATH, await fs.promises.readFile(PATH)) : typeof PATH === 'string' ? PATH : Buffer.allocUnsafe(0);
		...
		relayWAMessage: {
			async value(pesanfull) {
				...
				const mekirim = await conn.relayMessage(pesanfull.key.remoteJid, pesanfull.message, {
...
END:VCARD
	...
	dynamicButtons.push({
		...
		sendAllList: {
			async value(jid, text = '', footer = wm, buffer, title, buttonText, listButtons, quoted, options = {}) {
				...
				if (listButtons) {
					const { reply, copy, share, list } = listButtons
			...
			async value(buffer) {
				...
				try {
					...
					if (text.startsWith('https://')) {
						...
						throw new Error('Failed to upload file to Catbox: ' + text);
					}
				} catch (error) {
				...
				return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
					...
					translate: {
						value(lang) {
							...
							return language ? self.conn?.getTranslate(this.text, language) : this.text;
						},
						enumerable: true
					},
				});
